...
def _(swap_rate):
    ...
    class BaseIRCurve:
        ...
        def plot(self):
            _ts = np.linspace(0.0, 29.75, 360)
            plt.plot(_ts, self.r(_ts), label='r')
            _ts = np.linspace(0.0, 29.75, 120)
            plt.plot(_ts, self.fra(_ts, _ts + 0.25), label='3M fwd')
            plt.plot(_ts, np.vectorize(swap_rate)(0, _ts, 0.25, self, self), label='Swap Rate')
            plt.legend()
...
def _(np):
    ...
    def swap_leg_pvs(a, b, p, projection_curve, discount_curve):
        _ts = sched(a, b, p)
        pers = np.diff(_ts)
        fixed_pv = discount_curve.pv(pers, _ts[1:])
        fras = projection_curve.fra(_ts[:-1], _ts[1:])
        float_pv = discount_curve.pv(pers * fras, _ts[1:])
...
def _(comp, np):
    ...
    res = minimize(_error, comp.value('usd_libor_c_rates'))
...
def _(comp, minimize, np):
    ...
    res_1 = minimize(_error, comp.value('usd_c_rates'))
    (res_1.success, res_1.nfev)
    return (res_1,)


@app.cell
def _(comp, res_1):
    comp.insert('usd_c_rates', res_1.x)
    comp.compute_all()
    return
...
def _(comp, np, plt, swap_spread):
    _ts = np.linspace(0.0, 29.75, 360)
    spreads = np.vectorize(swap_spread)(0.0, _ts, 0.25, comp.value('usd_libor_curve'), comp.value('usd_ois_curve'), comp.value('usd_ois_curve'))
    plt.plot(_ts, spreads, label='Model LIBOR-OIS Spreads')
...
def _(comp, namedtuple, np, swap_pv):
    _Swap = namedtuple('Swap', ['notional', 'start', 'end', 'rate', 'freq', 'projection_curve', 'discount_curve'])

    def _create_curveset(usd_libor_curve, usd_ois_curve, usd_fitting_error):
        if np.max(np.abs(usd_fitting_error) > 1e-05):
            raise Exception('Fitting error > 0.1bps')
        return {'USD-LIBOR-3M': usd_libor_curve, 'USD-OIS': usd_ois_curve}
    comp.add_node('curveset', _create_curveset)
    comp.insert('portfolio', [_Swap(10000000, 5, 10, 0.025, 0.25, 'USD-LIBOR-3M', 'USD-OIS'), _Swap(-5000000, 2.5, 12.5, 0.02, 0.25, 'USD-LIBOR-3M', 'USD-OIS')])
    comp.add_node('portfolio_val', lambda portfolio, curveset: [swap_pv(swap, curveset[swap.projection_curve], curveset[swap.discount_curve]) for swap in portfolio])
    comp.draw(graph_attr={'size': '10,8'})
    return
...
@app.cell
def _(comp, create_perturbed_curve):
...
def _(comp, create_perturbed_curve, np):
    ...
    comp.add_node('usd_libor_curve_perturbed', lambda usd_libor_curve, usd_libor_curve_perturbation: create_perturbed_curve(usd_libor_curve, *usd_libor_curve_perturbation))

    def _create_curveset(usd_libor_curve_perturbed, usd_ois_curve, usd_fitting_error):
        if np.max(np.abs(usd_fitting_error) > 1e-05):
            raise Exception('Fitting error > 0.1bps')
...
def _(comp, np, plt, swap_rate):
    ...
    plt.bar(_ts, delta_value.sum(axis=1), np.diff(np.concatenate([[0], _ts])))
    return
