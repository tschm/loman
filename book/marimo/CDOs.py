...
def _(comp, loman, norm, np):
    ...
    def p_def_conds(M, p_defs, corr):
        return norm.cdf((norm.ppf(p_defs) - np.sqrt(corr) * _M) / np.sqrt(1 - corr))
    return
...
def _(comp, loman, np):
    ...
    def update_loss_distribution(ps, p_default, loss):
        ...
        _ps = _ps * (1 - p_default)
        _ps[loss_lower:] = _ps[loss_lower:] + p_default * (1 - loss_frac) * ps1[:-loss_lower]
        _ps[loss_upper:] = _ps[loss_upper:] + p_default * loss_frac * ps1[:-loss_upper]

    @loman.node(comp)
    def _ps(n_losses, ts, n_obligors, p_def_conds):
        _ps = init_loss_distribution(n_losses, len(ts))
        for i in range(n_obligors):
            update_loss_distribution(_ps, p_def_conds, 1)
        return _ps
    comp.compute_all()
    return
...
def _(comp, loman, np, plt):
    ...
    def expected_tranche_widths(ps, tranche_widths):
        return np.sum(_ps * tranche_widths, axis=0)
    comp.compute_all()
...
def _(comp, np):
    _n_abscissas = 40
    _Ms, _ws = np.polynomial.hermite.hermgauss(_n_abscissas)
    _Ms = _Ms * np.sqrt(2)
    _ws = _ws / np.sqrt(np.pi)
    default_leg_pv_1 = 0
    coupon_leg_1_pv_1 = 0
    coupon_leg_pv_1 = 0
    for _M, _w in zip(_Ms, _ws):
        comp.insert('M', _M)
        comp.compute_all()
        default_leg_pv_1 = default_leg_pv_1 + _w * comp.v.default_leg_pv
        coupon_leg_1_pv_1 = coupon_leg_1_pv_1 + _w * comp.v.coupon_leg_1_pv
        coupon_leg_pv_1 = coupon_leg_pv_1 + _w * comp.v.coupon_leg_pv
    return coupon_leg_1_pv_1, coupon_leg_pv_1, default_leg_pv_1
...
@app.cell
def _(default_leg_pv_1):
    default_leg_pv_1
    return


@app.cell
def _(coupon_leg_pv_1):
    coupon_leg_pv_1
    return


@app.cell
def _(coupon_leg_1_pv_1, default_leg_pv_1):
    default_leg_pv_1 / coupon_leg_1_pv_1
    return
...
def _(comp, np, plt):
    ...
    for _M, _w in zip(_Ms, _ws):
        ...
        comp.compute_all()
        if _ps is None:
            _ps = np.zeros_like(comp.v.ps)
        _ps = _ps + comp.v.ps * _w
    plt.plot(comp.v.losses[:, 0], _ps[:, 4], label=f't={comp.v.ts[4]}')
    plt.plot(comp.v.losses[:, 0], _ps[:, 8], label=f't={comp.v.ts[8]}')
    plt.plot(comp.v.losses[:, 0], _ps[:, 20], label=f't={comp.v.ts[20]}')
    plt.xlim(0, 600000)
    plt.legend()
    return
